...
AffyEngine::AffyEngine(const char* dataFilename, parameters p, bool noRS, bool genotype, int index)
{
	setValues(p);
	...
	{
		status = AffyEngine::ReadVCF(theFilename, noRS, genotype, index);
	}
	else
	{
		status = -1;
...
int AffyEngine::ReadVCF(const char* dataFilename, bool noRS, bool UseGenotype, int index)
{
	std::ifstream input(dataFilename);
	if (!input)
	{
	...
	else
	{
		...
		if (AffyEngine::searchVCFFile(dataFilename, UseGenotype, index) == false)
		{
			return -1;
		}
...
std::string AffyEngine::getGenotype(std::string item, int minimumReadDepth, bool genotype)
{
	...
	if (genotype == false)
	{
		if (subitems.size() <= AD || subitems.size() <= DP)
		{
			return answer;
		}
...
bool AffyEngine::searchVCFFile(const char* filename, bool genotype, int index)
{
	bool answer = false;
	std::string strData = "";
	int intStrArray = 0;
